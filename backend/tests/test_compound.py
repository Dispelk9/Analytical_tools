...
def test_compound_returns_compounds_and_duplicates(client, monkeypatch):
    ...
    monkeypatch.setattr("compound.requests.get", lambda *args, **kwargs: FakeResponse())
    monkeypatch.setattr("compound.pcp.Compound.from_cid", fake_from_cid)
    monkeypatch.setattr("compound.time.sleep", lambda *_: None)
