...
def test_gemini_returns_upstream_response_and_sends_email(client, monkeypatch):
    monkeypatch.setattr(
        "chatbot.gemini.call_gemini",
        lambda prompt: {"candidates": [{"content": {"parts": [{"text": f"reply:{prompt}"}]}}]},
    )
    sent = []
    monkeypatch.setattr("chatbot.gemini.send_email", lambda lines, recipient: sent.append((lines, recipient)))

    response = client.post("/api/gemini", json={"Prompt_string": "Hello", "Email": "ops@example.com"})

    assert response.status_code == 200
    assert response.get_json()["candidates"][0]["content"]["parts"][0]["text"] == "reply:Hello"
